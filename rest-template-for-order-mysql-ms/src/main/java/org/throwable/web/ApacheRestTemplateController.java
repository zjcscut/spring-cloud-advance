 ...
 */
@Controller
public class ApacheRestTemplateController {

    @Autowired
    @Qualifier(value = "apacheRestTemplate")
    private RestTemplate apacheRestTemplate;

    @GetMapping(value = "/apache/create-order", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    public Response<Order> createOrder() {
        ...
        ResponseEntity<Response<Order>> exchange = apacheRestTemplate.exchange("http://localhost:9096/order", HttpMethod.PUT,
                httpEntity, new ParameterizedTypeReference<Response<Order>>() {
                });
        return exchange.getBody();
    }

    @GetMapping(value = "/apache/get-order", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    public Response<Order> getOrder() {
        String orderId = "e75c874e-6761-4471-a4eb-99c53482f4fb";
        Map<String, Object> uriVariables = new HashMap<>();
        uriVariables.put("orderId", orderId);
        ResponseEntity<Response<Order>> exchange = apacheRestTemplate.exchange("http://localhost:9096/order?orderId={orderId}", HttpMethod.GET, null,
