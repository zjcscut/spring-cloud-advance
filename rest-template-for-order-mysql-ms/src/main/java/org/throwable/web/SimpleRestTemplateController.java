...
public class SimpleRestTemplateController {
    ...
    @GetMapping(value = "/simple/create-order", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    public Response<Order> createOrder() {
        ...
        ResponseEntity<Response<Order>> exchange = simpleRestTemplate.exchange("http://localhost:9096/order", HttpMethod.PUT,
                httpEntity, new ParameterizedTypeReference<Response<Order>>() {
                });
        return exchange.getBody();
    }

    @GetMapping(value = "/simple/get-order", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    public Response<Order> getOrder() {
        String orderId = "93c449f4-6118-44c0-94a2-9fbf50c654ba";
        Map<String, Object> uriVariables = new HashMap<>();
        uriVariables.put("orderId", orderId);
        ResponseEntity<Response<Order>> exchange = simpleRestTemplate.exchange("http://localhost:9096/order?orderId={orderId}", HttpMethod.GET, null,
